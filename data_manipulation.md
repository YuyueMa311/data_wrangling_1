...
```
    ...
    ## # A tibble: 49 × 5
    ##    gd0_weight gd18_weight gd_of_birth group litter_number  
    ##         <dbl>       <dbl>       <dbl> <chr> <chr>          
    ##  1       19.7        34.7          20 Con7  #85            
    ##  2       27          42            19 Con7  #1/2/95/2      
    ##  3       26          41.4          19 Con7  #5/5/3/83/3-3  
    ##  4       28.5        44.1          19 Con7  #5/4/2/95/2    
    ##  5       NA          NA            20 Con7  #4/2/95/3-3    
    ##  6       NA          NA            20 Con7  #2/2/95/3-2    
    ##  7       NA          NA            20 Con7  #1/5/3/83/3-3/2
    ##  8       NA          NA            20 Con8  #3/83/3-3      
    ##  9       NA          NA            20 Con8  #2/95/3        
    ## 10       28.5        NA            20 Con8  #3/5/2/2/95    
    ## # ℹ 39 more rows

rearrange by selecting, keep everything as listed; ‘relocate’ works the
same
...
filter out missing rows with `drop_na()` in the whole dataset or just
filter out missing rows under gd0_weight
...
Use `mutate()` to create or modify variables: create wt_gain and make
variable(group) lower case.
...
`arrange()` arranges things, descending order by `desc()` `head(...,10)`
